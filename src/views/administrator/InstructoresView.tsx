...
export default function InstructoresView() {
  ...
  const guardarInstructor = async (e: React.FormEvent) => {
    ...
    if (!nombreLimpio) {
      alert("Escribe el nombre del instructor.");
      return;
    }

    if (!especialidadLimpia) {
      alert("Escribe la especialidad del instructor.");
      return;
    }

    if (!correoLimpio) {
      alert("Escribe el correo del instructor.");
      return;
    }

    if (!telefonoLimpio) {
      alert("Escribe el teléfono del instructor.");
      return;
    }
    ...
    try {
      if (editing) {
        ...
        const rolRes = await getRolByNumero({
          rolId: 2,
        });
        ...
        if (!rolInstructor) {
          alert(
            "No se encontró el rol de instructor con rolId = 2."
          );
          return;
        }

        const passwordPlano = password.trim() || "123456";
        ...
        /*
         * El login usa bcrypt.compare(), por lo que aquí debemos guardar
         * un hash real y no la contraseña en texto plano.
         */
      ...
      alert(
        "Error al guardar instructor. Revisa la consola."
      );
    } finally {
      setGuardando(false);
    }
  ...
  const eliminarInstructor = async (inst: Instructor) => {
    if (
      !window.confirm(
        `¿Seguro que deseas eliminar al instructor ${inst.nombre}?`
      )
    ) {
      return;
    }
  ...
  const cambiarEstado = async (inst: Instructor) => {
    try {
      await updateInstructor({
        ...
        correo: inst.email.toLowerCase().trim(),
  ...
  const filtered = instructores.filter((i) => {
    ...
    return (
      ...
      i.especialidad.toLowerCase().includes(termino) ||
        ...
        <div className="ucv-clases-header">
          <div>
            ...
            <div className="ucv-clases-header__sub">
              {filtered.length} instructores en total
            </div>
          </div>
                    ...
                    <tr key={inst.id}>
                      ...
                      <td>
                        <span
                          ...
                          style={{
                            background:
                              ESTADO_STYLES[inst.estado].bg,
                            color:
                              ESTADO_STYLES[inst.estado]
                                .color,
                          }}
                        >
                          {inst.estado}
                      ...
                      <td>
                        <div className="ucv-actions">
                          ...
                          <button
                            type="button"
                            className="ucv-action-btn ucv-action-btn--toggle"
          ...
          <div className="ucv-table-footer">
            <span className="ucv-table-footer__info">
              ...
              {filtered.length === 0
                ? 0
                : (page - 1) * perPage + 1}{" "}
              a {Math.min(page * perPage, filtered.length)} de{" "}
          ...
          >
            ...
            {!editing && (
              <div style={{ display: "grid", gap: 6 }}>
                ...
                >
                  La contraseña se almacenará protegida.
                </span>
              </div>
            )}
