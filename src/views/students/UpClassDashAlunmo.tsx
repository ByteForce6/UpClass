      ...
      <div className="uc-stack-lg">
        ...
        <div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            {[
              {label:"Cursos disponibles", sub:"Ver cupo y precios",  key:"catalogoCursos"   as View, icon:"◈"},
              {label:"Horarios",           sub:"Consultar calendario", key:"catalogoHorarios" as View, icon:"◷"},
            ].map((item)=>(
              <button
                key={item.key}
                onClick={()=>onNav(item.key)}
                style={{display:"flex",alignItems:"center",gap:14,padding:"16px 18px",background:"#fff",border:"1px solid #e0dbd4",textAlign:"left" as const,cursor:"pointer",transition:"all .2s"}}
                onMouseEnter={e=>(e.currentTarget.style.borderColor="#111")}
                onMouseLeave={e=>(e.currentTarget.style.borderColor="#e0dbd4")}
              >
                <span style={{fontSize:22,color:"#111"}}>{item.icon}</span>
                <div>
                  <p style={{fontFamily:"var(--dr)",fontSize:14,fontWeight:700,color:"#111",margin:"0 0 2px"}}>{item.label}</p>
                  <p style={{fontFamily:"var(--ds)",fontSize:11,color:"#aaa",margin:0}}>{item.sub}</p>
                </div>
              </button>
            ))}
          </div>
      ...
      <>
        <style>{`
          ...
          *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
          ...
          .uc-main{flex:1;display:flex;flex-direction:column;min-width:0;overflow:hidden;margin-left:232px;}
          ...
          /* Nav — este es el que scrollea si hay overflow */
        ...
        <div className="uc-shell">
          ...
          <div
            ...
            style={{position:"fixed",inset:0,zIndex:190,display:sidebar?"block":"none"}}
          />
